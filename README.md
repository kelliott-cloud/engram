...
```bash
openclaw plugins install @applied-leverage/engram
```

If you're running from a local OpenClaw checkout:

```bash
pnpm openclaw plugins install @applied-leverage/engram
```

For local plugin development, build and link your working copy:

```bash
git clone https://github.com/kelliott-cloud/engram.git
cd engram
npm install
npm run build
