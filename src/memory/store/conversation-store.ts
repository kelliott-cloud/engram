...
export class ConversationStore {
  ...
  private searchRegex(
    ...
    // Input hardening: regex patterns are user-controlled (exposed via the
    // POST /memory/search route), so bound complexity to prevent ReDoS and
    // OOM. SQLite has no native POSIX regex; fetch candidates and filter in JS.
    const safePattern = ConversationStore.compileSafeRegex(pattern);
    ...
    // Cap the SQL scan so we don't materialize the entire messages table.
    // Use `limit * 64 + 1024` as a heuristic — enough to find matches in
    // sparse hits without blowing memory on multi-million-row databases.
    const sqlScanLimit = Math.max(1024, limit * 64);
    ...
    const deadline = Date.now() + ConversationStore.REGEX_SCAN_BUDGET_MS;
    ...
    for (const row of rows) {
      ...
      if (Date.now() > deadline) {
        // Budget exceeded — bail out with whatever we found. Surfacing a
        // partial result is preferable to letting a malicious pattern
        // hold the event loop indefinitely.
        break;
      }
      // Cap per-row content length to bound per-match work; matches longer
      // than this are unlikely to be useful as snippets anyway.
      const content =
        row.content.length > ConversationStore.REGEX_CONTENT_MAX
          ? row.content.slice(0, ConversationStore.REGEX_CONTENT_MAX)
          : row.content;
      let match: RegExpExecArray | null = null;
      try {
        match = safePattern.exec(content);
      } catch {
        // Defensive: pathological inputs shouldn't crash the whole search.
        match = null;
      }
      if (match) {
        results.push({
  ...
  // ── Regex hardening (ReDoS guard) ────────────────────────────────────────
  private static readonly REGEX_PATTERN_MAX = 256;
  private static readonly REGEX_CONTENT_MAX = 64_000;
  private static readonly REGEX_SCAN_BUDGET_MS = 250;

  /**
   * Compile a user-supplied regex with ReDoS-resistant constraints:
   * - cap the source length
   * - reject obvious catastrophic-backtracking shapes (nested quantifiers)
   * - reject the inline flag form `(?i)` etc. (not standard JS, and a vector
   *   for confusion)
   *
   * Throws a clear error so the HTTP layer can return a 400 instead of a 500.
   */
  static compileSafeRegex(pattern: string): RegExp {
    ...
    if (pattern.length > ConversationStore.REGEX_PATTERN_MAX) {
      throw new Error(
        `regex pattern too long (max ${ConversationStore.REGEX_PATTERN_MAX} chars)`,
      );
    }
    // Classic catastrophic-backtracking shapes: (X+)+ , (X*)*, (X+)*, (X*)+
    // and nested quantifier groups like (a|aa)+ that explode on near-misses.
    if (/\([^)]*[+*]\)[+*]/.test(pattern)) {
      throw new Error("regex pattern contains nested quantifiers (ReDoS risk)");
    }
    // Disallow inline flag groups like (?i) — these are PCRE syntax, not JS.
    if (/\(\?[a-zA-Z]+\)/.test(pattern)) {
      throw new Error("regex pattern contains unsupported inline flag group");
    }
    try {
