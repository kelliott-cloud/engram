  ...
  return async (req: IncomingMessage, res: ServerResponse): Promise<boolean> => {
    try {
      ...
      if (pathname === "/memory/search" && method === "POST") {
        ...
        // Strictly validate conversationId — `Number()` silently coerces
        // booleans/arrays/empty strings; require a number/string that parses
        ...
        if (conversationIdRaw != null) {
          if (
            ...
            sendJson(res, 400, { detail: "invalid conversationId" });
            return true;
          }
        ...
        try {
          ...
          // Surface regex/input validation errors as 400 rather than 500.
          const msg = err instanceof Error ? err.message : String(err);
          if (/regex|pattern/i.test(msg)) {
            sendJson(res, 400, { detail: msg });
          } else {
            throw err;
          }
        } finally {
