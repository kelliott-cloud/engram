...
const writeManagedText = (
  ...
  const normalized = toPosix(relPath);
  // Guard against path traversal — `relPath` is composed from session/agent
  // ids and titles which are sanitized upstream, but defense-in-depth is
  // cheap. Reject anything that resolves outside `state.mirrorRoot`.
  if (
    !normalized ||
    ...
    throw new Error(`vault write rejected: unsafe path ${JSON.stringify(relPath)}`);
  }
  ...
  // Final containment check: the resolved absolute path MUST live under
  // mirrorRoot. Catches edge cases where the normalized form passed the
  // pattern checks but still escapes.
  const resolvedTarget = path.resolve(targetPath);
  const resolvedRoot = path.resolve(state.mirrorRoot);
  if (
    resolvedTarget !== resolvedRoot &&
    !resolvedTarget.startsWith(resolvedRoot + path.sep)
  ) {
    throw new Error(`vault write rejected: ${resolvedTarget} escapes ${resolvedRoot}`);
  }
