...
const lcmPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    if (!gatewayToken) {
      api.logger.warn(
        "[lcm] gateway.auth.token is not configured; /memory HTTP routes will rely on OpenClaw gateway auth only",
      );
    }
    ...
    // OpenClaw's plugin SDK exposes `registerHttpRoute` (with path + auth scope)
    // rather than a global handler. Use a prefix match on `/memory` so all
    // `/memory/*` routes resolve to the handler's internal router.
    api.registerHttpRoute({
      path: "/memory",
      match: "prefix",
      auth: "gateway",
      handler: lcmHttpHandler,
    });
